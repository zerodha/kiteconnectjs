...
var KiteTicker = function(api_key, user_id, public_token, address) {
	...
	this.connect = function() {
		...
		ws.onopen = function() {
			...
			read_timer = setInterval(function() {
				if((new Date() - last_read ) / 1000 >= read_timeout) {
					// reset currentWsUrl incase current connection timesout
					// This is determined by the last heart beat received time
		...
		ws.onmessage = function(e) {
			...
			// Set last readtime to check for connection timeout
			last_read = new Date();
		};
