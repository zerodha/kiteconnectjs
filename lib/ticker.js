 ...
 * @param {string} params.user_id Zerodha client id (ex. DV0006)
...
var KiteTicker = function(params) {
	var root = params.root || "wss://websocket.kite.trade/";
	...
	this.connect = function() {
		...
		var url = root + "?api_key=" + params.api_key + "&user_id=" + params.user_id +
	 ...
	 * disconnect - when socket connection is disconnected.
	 * reconnecting - When reconnecting (Reconnecting interval and current re-connection count as arguments respectively).
	...
	function parseBinary(binpacks) {
		...
		for(var n=0; n<packets.length; n++) {
			var bin = packets[n],
				t = buf2long(bin.slice(0, 4)),
				instrument_token = t >> 8,
				segment = t & 0xff;
			...
			// Parse indices quote and full mode
                ...
                if (bin.byteLength === 32) {
                    tick.timestamp = buf2long(bin.slice(28, 32));
				}
				...
				if (bin.byteLength === 184) {
					tick.last_trade_time = buf2long(bin.slice(44, 48));
					tick.timestamp = buf2long(bin.slice(60, 64));
					tick.oi = buf2long(bin.slice(48, 52));
                    tick.oi_high = buf2long(bin.slice(52, 56));
					tick.oi_low = buf2long(bin.slice(56, 60));
					tick.depth = {
						buy: [],
						sell: []
					};
