...
const KiteTicker = function(params) {
  ...
  let reconnectMaxDelay = 0;
  let reconnectMaxTries = 0;
  ...
  this.connect = function() {
    ...
    ws.onopen = function() {
      ...
      if (!currentWsUrl) currentWsUrl = this.url;
    ...
    ws.onclose = function(e) {
      ...
      if (currentWsUrl && this.url != currentWsUrl) return;
  ...
  function autoReconnect(t, maxRetry, maxDelay) {
    autoReconnect = t == true;

    // Set default values
    maxRetry = maxRetry || defaultReconnectMaxRetries;
    maxDelay = maxDelay || defaultReconnectMaxDelay;

    // Set reconnect constraints
    reconnectMaxTries =
      maxRetry >= maximumReconnectMaxRetries ?
        maximumReconnectMaxRetries :
        maxRetry;
    reconnectMaxDelay =
      maxDelay <= minimumReconnectMaxDelay ?
  ...
  function triggerDisconnect(e) {
    ...
    if (autoReconnect) attemptReconnection();
  }
    ...
    for (let n = 0; n < packets.length; n++) {
      ...
      if (bin.byteLength === 8) {
        ...
        if (bin.byteLength === 184) {
          ...
          const lastTradeTime = buf2long(bin.slice(44, 48));
          if (lastTradeTime) {
  ...
  function attemptReconnection() {
    ...
    if (currentReconnectionCount > 0) {
      lastReconnectInterval = Math.pow(2, current_reconnection_count);
