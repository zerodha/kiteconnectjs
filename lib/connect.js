...
class KiteConnect {
  ...
  async generateSession(requestToken, apiSecret) {
    const checksum = sha256(
        this.api_key + requestToken + apiSecret,
    ).toString();
    try {
   ...
   */
  invalidateAccessToken = function(accessToken) {
   ...
   * @param {string} api_secret API secret issued with the API key.
   */
  renewAccessToken = async function(refreshToken, apiSecret) {
    const checksum = sha256(
        this.api_key + refreshToken + apiSecret,
    ).toString();
    try {
      const resp = await this._post('api.token.renew', {
        api_key: this.api_key,
        refreshToken,
        checksum,
      });
...
regular).
     * @param {string} [params.parent_order_id] Parent order id incase of multilegged orders.
     */
  cancelOrder(variety, orderId, params) {
    params = params || {};
    params.variety = variety;
   ...
   * @param {string} [params.parent_order_id] Parent order id incase of multilegged orders.
   */
  exitOrder(variety, orderId, params) {
  ...
  getOrderHistory(orderId) {
    return this._get('order.info', {order_id: orderId}, null, formatResponse);
  }
   ...
   * @memberOf KiteConnect
   * @instance
   * @param {string} orderId ID of the order whose trades are to be retrieved.
   */
  getOrderTrades(orderId) {
    return this._get('order.trades', {order_id: orderId}, null, formatResponse);
  }
  ...
  orderMargins(orders, mode = null) {
    return this._post('order.margins', orders, null, null, true, {mode: mode});
  }
   ...
   * @param {string} mode    (optional) Compact mode will only give the total margins
   */
  orderBasketMargins(orders, considerPositions = true, mode = null) {
   ...
   * @param {string|Date} fromDate From date (String in format of 'yyyy-mm-dd HH:MM:SS' or Date object).
   * @param {string|Date} toDate To date (String in format of 'yyyy-mm-dd HH:MM:SS' or Date object).
  ...
  getHistoricalData(
    ...
    oi = oi ? 1 : 0;
    if (typeof toDate === 'object') toDate = _getDateTimeString(toDate);
    if (typeof fromDate === 'object') {
      fromDate = _getDateTimeString(fromDate);
    }
   ...
   */
  getTriggerRange(transactionType, instruments) {
  ...
  getMFSIPS(sipId) {
    if (sipId) {
      return this._get('mf.sip.info', {sip_id: sipId}, null, formatResponse);
    } else {
   ...
   */
  cancelMFSIP(sipId) {
    return this._delete('mf.sip.cancel', {sip_id: sipId});
  }
   ...
   * @memberOf KiteConnect
   * @instance
   * @param {string} params.trigger_type GTT type, its either `this.GTT_TYPE_OCO` or `this.GTT_TYPE_SINGLE`.
   * @param {string} params.tradingsymbol Tradingsymbol of the instrument (ex. RELIANCE, INFY).
   * @param {string} params.exchange Exchange in which instrument is listed (NSE, BSE, NFO, BFO, CDS, MCX).
   ...
   */
  deleteGTT(triggerId) {
    return this._delete('gtt.delete', {trigger_id: triggerId}, null, null);
  }
   ...
   * @return {bool} Return true if checksum matches else false
   * @throws Throws an error if the @postback_data or @api_secret is invalid
   */
  validatePostback(postbackData, apiSecret) {
    if (
      !postbackData ||
      !postbackData.checksum ||
      !postbackData.order_id ||
      !postbackData.order_timestamp ||
      !apiSecret
    ) {
    ...
    const inputString =
      postbackData.order_id + postbackData.order_timestamp + apiSecret;
    let checksum;
    try {
    ...
    if (postbackData.checksum === checksum) {
      return true;
    } else {
      return false;
    }
