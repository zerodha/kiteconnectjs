...
class KiteConnect {
  ...
  async generateSession(requestToken, apiSecret) {
    const checksum = sha256(
        self.api_key + requestToken + apiSecret,
    ).toString();
    try {
  ...
  invalidateAccessToken = function(accessToken) {
    accessToken = accessToken || self.access_token;
  ...
  renewAccessToken = async function(refreshToken, apiSecret) {
    ...
    try {
      const resp = await _post('api.token.renew', {
...
regular).
  ...
  getOrderHistory(orderId) {
    return _get('order.info', {order_id: orderId}, null, formatResponse);
  }
  ...
  getOrderTrades(orderId) {
    return _get('order.trades', {order_id: orderId}, null, formatResponse);
  }
  ...
  orderMargins(orders, mode = null) {
    return _post('order.margins', orders, null, null, true, {mode: mode});
  }
  ...
  getMFSIPS(sipId) {
    if (sipId) {
      return _get('mf.sip.info', {sip_id: sipId}, null, formatResponse);
    } else {
   ...
   */
  cancelMFSIP(sipId) {
    return _delete('mf.sip.cancel', {sip_id: sipId});
  }
  ...
  deleteGTT(trigger_id) {
    return _delete('gtt.delete', {trigger_id: triggerId}, null, null);
  }
...
function parseHistorical(jsonData) {
  ...
  const results = [];
  for (const i = 0; i < jsonData.data.candles.length; i++) {
