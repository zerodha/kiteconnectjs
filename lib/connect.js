 ...
 * @param {string} [params.login="https://kite.trade/connect/login"] Kite connect login url
...
var KiteConnect = function(params) {
	...
	self.sessionExpiryHook = null;
  	...
  	var userAgent = '';  // User agent to be sent with every request
	...
	// Set contexnt type as form encoded for PUT and POST
	...
	*/
	 ...
	 * @memberOf KiteConnect
	 * @instance
	 * @param {string} [order_id] ID of the order (optional) whose order details to be retrieved.
	...
	self.historical = function(params) {
		return _get("market.historical", params, parseHistorical);
	};
	...
	function request(route, method, params, responseType) {
		// Check access token
		if (!responseType) responseType = "json";
		if(self.api_key) params.api_key = self.api_key;
		if(self.access_token) params.access_token = self.access_token;
