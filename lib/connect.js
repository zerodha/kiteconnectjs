...
regular).
	 ...
	 * @param {string|Date} from From date (String in format of 'yyyy-mm-dd HH:MM:SS' or Date object).
	 * @param {string|Date} to To date (String in format of 'yyyy-mm-dd HH:MM:SS' or Date object).
	 * @param {bool}  [continuous=false] is a bool flag to get continuous data for futures and options instruments. Defaults to false.
	 */
	self.getHistoricalData = function(instrument_token, interval, from, to, continuous) {
		continuous = continuous ? 1 : 0;
		if (typeof to === "object") to = getDateTimeString(to)
		if (typeof from === "object") from = getDateTimeString(from)
