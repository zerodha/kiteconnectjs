...
var KiteConnect = function(params) {
	...
	*/
	...
	self.invalidateAccessToken = function(access_token) {
		...
		return _delete("api.token.invalidate", params);
	};
	...
	self.invalidateRefreshToken = function(refresh_token) {
		return _delete("api.token.renew.invalidate", {
			refresh_token: refresh_token
		});
	};
	...
	self.getOrderHistory = function(order_id) {
		return  _get("order.info", {"order_id": order_id});
	};
	...
	self.getOrderTrades = function(order_id) {
		return  _get("order.trades", {"order_id": order_id});
	};
	 ...
	 * @param {string|Date} params.from From date (String in format of 'yyyy-mm-dd HH:MM:SS' or Date object).
	 * @param {string|Date} params.to To date (String in format of 'yyyy-mm-dd HH:MM:SS' or Date object).
	 * @param {string} params.interval candle interval (minute, day, 5 minute etc.)
	 * @param {bool}   [params.continuous=false] is a bool flag to get continuous data for futures and options instruments. Defaults to false.
	 */
	self.getHistoricalData = function(params) {
		params.continuous = params.continuous ? 1 : 0;
		if (typeof params.to === "object") params.to = getDateTimeString(params.to)
		if (typeof params.from === "object") params.from = getDateTimeString(params.from)
	 ...
	 */
	self.getTriggerRange = function(exchange, tradingsymbol, transaction_type) {
